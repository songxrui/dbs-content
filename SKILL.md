---
name: dbs-content
description: "内容全周期诊断。选题→结构→初稿→润色→发布全管线评估，含内容形式匹配(图文/短视频/长视频/直播/文章)、5条核心哲学(文字洁癖/精神控制/投入×理解/先产品后内容/搞清楚说清楚)。触发词：/dbs-content、内容诊断、这个内容怎么做、帮我看看文案、内容方向、选题评估、形式匹配。不适用场景：仅开头优化→dbs-hook、仅标题优化→dbs-xhs-title、单篇写作→khazix-writer。正例：'帮我诊断这个选题'→触发; '这个内容方向对不对'→触发。反例：'帮我写个开头'→不触发→dbs-hook。"
...
| 失败模式 | 原因 | 兜底动作 |
|---------|------|---------|
| 用户没有明确选题 | 用户只有模糊方向 | 先帮用户理清选题(引导说出"想表达什么核心观点")再进入诊断 |
| 内容形式匹配错误 | 选题特性与推荐形式不匹配 | 回Phase 2重新评估，给2-3种可选形式对比优劣 |
| 诊断后用户无法执行 | 诊断建议过于抽象(如"加强共鸣") | 每条建议配>=1个具体可操作的动作+示例 |
